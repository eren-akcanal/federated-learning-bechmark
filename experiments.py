...
def get_args():
    ...
    # -----------------------------------------------------------------------
    # NEW: number of parallel worker threads for simulated client training.
    #   --num_workers 1   → original sequential behaviour (safe default)
    #   --num_workers 4   → 4 clients train concurrently inside a ThreadPool
    #   --num_workers -1  → use len(selected) threads (fully parallel)
    # Note: threads share the GIL but PyTorch releases it for most GPU/CPU
    # tensor ops, so you get real overlap.  For CPU-only runs consider
    # torch.multiprocessing instead (requires picklable nets).
    # -----------------------------------------------------------------------
    parser.add_argument('--num_workers', type=int, default=1,
                        help='Number of parallel threads for simulated client training. '
                             '-1 means one thread per selected client (fully parallel).')
...
# ---------------------------------------------------------------------------
# Helper: resolve the actual thread-pool size given --num_workers and the
# number of clients selected this round.
# ---------------------------------------------------------------------------
def _pool_size(num_workers_arg: int, n_selected: int) -> int:
    """Return the actual ThreadPoolExecutor max_workers to use."""
    if num_workers_arg == -1:
        return n_selected          # fully parallel — one thread per client
    return max(1, num_workers_arg) # at least 1 (sequential if 1)
...
def local_train_net(nets, selected, args, net_dataidx_map, test_dl=None,
                    device="cpu", train_dl_cache=None):
    avg_acc = 0.0
    results = {}   # net_id -> testacc

    def _train_one(net_id):
        net = nets[net_id]
        ...
        return net_id, testacc

    pool_size = _pool_size(args.num_workers, len(selected))
    if pool_size == 1:
        # Sequential path — zero overhead, identical to original
        for net_id in selected:
            nid, tacc = _train_one(net_id)
            results[nid] = tacc
    else:
        with ThreadPoolExecutor(max_workers=pool_size) as pool:
            futures = {pool.submit(_train_one, net_id): net_id for net_id in selected}
            for fut in as_completed(futures):
                nid, tacc = fut.result()
                results[nid] = tacc

    avg_acc = sum(results.values()) / len(selected)
...
def local_train_net_fedprox(nets, selected, global_model, args, net_dataidx_map,
                             test_dl=None, device="cpu", train_dl_cache=None):
    results = {}
...
# ---------------------------------------------------------------------------
# local_train_net_scaffold
# ---------------------------------------------------------------------------
def local_train_net_scaffold(nets, selected, global_model, c_nets, c_global, args,
                              net_dataidx_map, test_dl=None, device="cpu", train_dl_cache=None):
    results = {}
    ...
    # c_delta_para results keyed by net_id
    c_delta_results = {}
    ...
    def _train_one(net_id):
        ...
        logger.info("net %d final test acc %f" % (net_id, testacc))
        return net_id, testacc, c_delta_para

    pool_size = _pool_size(args.num_workers, len(selected))
    if pool_size == 1:
        for net_id in selected:
            nid, tacc, cdp = _train_one(net_id)
            results[nid] = tacc
            c_delta_results[nid] = cdp
    else:
        with ThreadPoolExecutor(max_workers=pool_size) as pool:
            futures = {pool.submit(_train_one, net_id): net_id for net_id in selected}
            for fut in as_completed(futures):
                nid, tacc, cdp = fut.result()
                results[nid] = tacc
                c_delta_results[nid] = cdp

    # Accumulate c_delta on main thread (safe — no concurrent writes)
    for net_id in selected:
        for key in total_delta:
            total_delta[key] += c_delta_results[net_id][key]
...
# ---------------------------------------------------------------------------
# local_train_net_precond  (fedmuon / fedadamw)  — PRIMARY TARGET
# ---------------------------------------------------------------------------
def local_train_net_precond(nets, selected, global_model, args, net_dataidx_map,
                             ps_c, v, alg, test_dl=None, device="cpu",
                             train_dl_cache=None, step=None):
    """
    Parallelised version of the preconditioned local training loop.

    Each selected client trains independently; results (delta_w, momentum,
    dataset size) are collected into lists in the same order as `selected`
    for the caller's aggregation logic.
    """
    global_model.to(device)

    # Per-client momentum snapshots from previous round — read-only during training.
    # We hand each worker its own shallow copy of v to avoid sharing state.
    # (Values are tensors; the dict shell is new but tensors are shared read-only.)
    def _get_client_v(net_id):
        # Each client gets its own dict view — tensors are only read, not written,
        # during training; the updated copy is returned in m_i.
        return dict(v)

    def _get_client_ps_c(net_id):
        return dict(ps_c)
    ...
    def _train_one(net_id):
        ...
        client_ps_c = _get_client_ps_c(net_id)

        if alg == 'fedmuon':
            trainacc, testacc, d_i, m_i = train_net_muon(
                net_id, net, global_model, train_dl_local, test_dl, n_epoch,
                args.lr, args.optimizer, args, client_ps_c, client_v, device, step)
        elif alg == 'fedadamw':
            trainacc, testacc, d_i, m_i = train_net_adamw(
                net_id, net, global_model, train_dl_local, test_dl, n_epoch,
                args.lr, args.optimizer, args, client_ps_c, client_v, device, step)
        else:
            raise ValueError(f"Unknown alg for precond training: {alg}")

        n_i = len(train_dl_local.dataset)
        logger.info("net %d final test acc %f" % (net_id, testacc))
        return net_id, testacc, d_i, m_i, n_i

    # Determine parallelism
    pool_size = _pool_size(args.num_workers, len(selected))

    # Collect results indexed by position in `selected` (order matters for
    # the caller's aggregation, so we sort by net_id after collection).
    result_map = {}   # net_id -> (testacc, d_i, m_i, n_i)

    if pool_size == 1:
        # Sequential — identical behaviour to original, zero overhead
        for net_id in selected:
            nid, tacc, d_i, m_i, n_i = _train_one(net_id)
            result_map[nid] = (tacc, d_i, m_i, n_i)
    else:
        with ThreadPoolExecutor(max_workers=pool_size) as pool:
            futures = {pool.submit(_train_one, net_id): net_id for net_id in selected}
            for fut in as_completed(futures):
                nid, tacc, d_i, m_i, n_i = fut.result()
                result_map[nid] = (tacc, d_i, m_i, n_i)

    # Re-assemble ordered lists (same order as `selected`) for the caller
    avg_acc = 0.0
    m_list, d_list, n_list = [], [], []
    for net_id in selected:
        tacc, d_i, m_i, n_i = result_map[net_id]
        m_list.append(m_i)
        d_list.append(d_i)
        n_list.append(n_i)
        avg_acc += tacc
    ...
    return list(nets.values()), m_list, d_list, n_list


# ---------------------------------------------------------------------------
# local_train_net_fednova
# ---------------------------------------------------------------------------
def local_train_net_fednova(nets, selected, global_model, args, net_dataidx_map,
                             test_dl=None, device="cpu", train_dl_cache=None):
    result_map = {}
    global_model.to(device)
    ...
    def _train_one(net_id):
        ...
        n_i = len(train_dl_local.dataset)
        logger.info("net %d final test acc %f" % (net_id, testacc))
        return net_id, testacc, a_i, d_i, n_i

    pool_size = _pool_size(args.num_workers, len(selected))
    if pool_size == 1:
        for net_id in selected:
            nid, tacc, a_i, d_i, n_i = _train_one(net_id)
            result_map[nid] = (tacc, a_i, d_i, n_i)
    else:
        with ThreadPoolExecutor(max_workers=pool_size) as pool:
            futures = {pool.submit(_train_one, net_id): net_id for net_id in selected}
            for fut in as_completed(futures):
                nid, tacc, a_i, d_i, n_i = fut.result()
                result_map[nid] = (tacc, a_i, d_i, n_i)

    avg_acc = 0.0
    a_list, d_list, n_list = [], [], []
    for net_id in selected:
        tacc, a_i, d_i, n_i = result_map[net_id]
        a_list.append(a_i)
        d_list.append(d_i)
        n_list.append(n_i)
        avg_acc += tacc
    ...
    return list(nets.values()), a_list, d_list, n_list


# ---------------------------------------------------------------------------
# local_train_net_moon
# ---------------------------------------------------------------------------
def local_train_net_moon(nets, selected, args, net_dataidx_map, test_dl=None,
                          global_model=None, prev_model_pool=None, round=None,
                          device="cpu", train_dl_cache=None):
    results = {}
    global_model.to(device)
...
if __name__ == '__main__':
    ...
    # Pre-build one DataLoader per client
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            local_train_net(nets, selected, args, net_dataidx_map, test_dl=test_dl_global,
                            device=device, train_dl_cache=client_train_loaders)
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_fedprox(nets, selected, global_model, args, net_dataidx_map,
                                    test_dl=test_dl_global, device=device, train_dl_cache=client_train_loaders)
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_scaffold(nets, selected, global_model, c_nets, c_global, args,
                                     net_dataidx_map, test_dl=test_dl_global, device=device,
        ...
        logger.info("Initializing nets for FedMuon/FedAdamW")
        nets, local_model_meta_data, layer_type = init_nets(args.net_config, args.dropout_p, args.n_parties, args)
        global_models, global_model_meta_data, global_layer_type = init_nets(args.net_config, 0, 1, args)
        global_model = global_models[0]
        ...
        for round in range(args.comm_round):
            ...
            _, a_list, d_list, n_list = local_train_net_fednova(
                nets, selected, global_model, args, net_dataidx_map,
                test_dl=test_dl_global, device=device, train_dl_cache=client_train_loaders)
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_moon(nets, selected, args, net_dataidx_map,
                                 test_dl=test_dl_global, global_model=global_model,
        ...
        nets, local_model_meta_data, layer_type = init_nets(args.net_config, args.dropout_p, args.n_parties, args)
        arr = np.arange(args.n_parties)
        local_train_net(nets, arr, args, net_dataidx_map, test_dl=test_dl_global,
                        device=device, train_dl_cache=client_train_loaders)
